...
- The 2D↔3D toggle morphs with **duration 0** (instant) on purpose. Cesium's *animated* morph swings the camera out to a global view and back — the toggle saves the camera before the morph and restores it on `morphComplete` (`handleSceneModeChange`), so an instant morph keeps you in place; don't re-add a non-zero duration
