...
- **Live location — Phase 2 Stage 1 COMPLETE + DEPLOYED** (2026-07-07/08) — the web foreground live-GPS pipeline is shipped, verified, and **live in prod (2026-07-08)** across 4 vertical slices: (01) live marker device→SSE→watcher, (02) honest liveness (fresh/stale/unavailable/not-shared) + mid-trip rehydrate, (03) per-trip privacy toggle (with-trip/owner-only/off) + server guard (`shouldBroadcastPosition`, `PATCH /sharing`), (04) `plannedBasemap` save/rehydrate + drift-only live-view framing (killed the per-fix camera yank). Coarse battery cadence (sample ~3 min, broadcast per-fix, DB persist ~10 min, stale ~10 min). Four pure seams TDD'd under `node --test`. **Deploy:** backend live on Linode (`live-privacy.js` + `PATCH /sharing` + position guard); frontend pushed to `main` (commit `2a4b8e2`) — this also brought the npm-Cesium migration + terrain-accuracy streams to prod, which had been merged locally but never pushed (`origin/main` was 21 commits behind). En route, fixed a deploy.sh bundle bug that 502'd the backend — it shipped `backend-server.js` without its sibling ESM modules ([journal 07-08 deploy-bundle](../journal/2026-07-08-deploy-bundle-missing-sibling-modules.md)). See [plans/live-location.md](../plans/live-location.md). Next bet: Stage 2 (Capacitor background).
