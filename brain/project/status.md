...
- **Live location — Phase 2 Stage 1 COMPLETE** (2026-07-07/08) — the web foreground live-GPS pipeline is shipped + verified across 4 vertical slices: (01) live marker device→SSE→watcher, (02) honest liveness (fresh/stale/unavailable/not-shared) + mid-trip rehydrate, (03) per-trip privacy toggle (with-trip/owner-only/off) + server guard (`shouldBroadcastPosition`, `PATCH /sharing`), (04) `plannedBasemap` save/rehydrate + drift-only live-view framing (killed the per-fix camera yank). Coarse battery cadence (sample ~3 min, broadcast per-fix, DB persist ~10 min, stale ~10 min). Four pure seams TDD'd under `node --test`. **Not yet deployed to Linode** — backend adds `live-privacy.js` + `PATCH /sharing` + position guard; deploy before it's live for real watchers. See [plans/live-location.md](../plans/live-location.md). Next bet: Stage 2 (Capacitor background).
