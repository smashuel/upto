...
## Shipped (2026-07-31 — map notes, and the one-route rule)

- `notes?: TripNote[]` on the TripLink. Same JSONB passthrough as routes and `plannedBasemap` — no schema or endpoint change. Wizard captures them via `onNoteAdded` → form state; `TripPlanningMap` takes `initialNotes`; ActiveTrip and PublicAdventureView pass them so a share-link recipient sees the pins and can tap one for its detail (title as the Cesium entity `name` — the info-box header — with type, body and coordinates as the description).
- `NoteManager.onAdded` emits `SerializableNote`, never a `MapNote`: the latter carries live Cesium objects that cannot survive React form state or `JSON.stringify` on the way to the TripLink.
- Notes are stored at **zero height** and rendered as a bottom-anchored billboard clamped to ground. Storing the picked terrain height left the marker floating above the surface, where it slid laterally against the map as the camera zoomed.
- **`routes` now holds at most one route** — see [ADR 018](../decisions/018-one-route-per-triplink.md). It stays an array for backward compatibility (TripLinks written earlier may hold several, are not rewritten, and still render in full on read-only maps) and because the deferred multi-leg direction will want it.
