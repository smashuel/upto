...
| [03](../../.scratch/live-location/issues/03-privacy-toggle-and-server-guard.md) | Privacy toggle + server guard (with-trip/owner-only/off, consent chip, contextual permission) | `shouldBroadcastPosition` | 01, 02 | **IMPLEMENTED 2026-07-07** — guard TDD'd (6 cases), `PATCH /sharing` persists to JSONB, position endpoint gates live fixes on stored liveSharing, ActiveTrip 3-way chip enforces by-not-publishing (off=no sample, owner-only=local render/no POST, with-trip=POST), flip-away fires `unavailable`. Unit+tsc+lint green; **two-device verify pending** |
