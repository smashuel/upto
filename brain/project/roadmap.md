...
- [ ] **My Trips page + Persistence Phase 4 tie-up** — in-app list of active/past TripLinks per account, backed by a new `GET /api/triplinks` list endpoint, with the localStorage dual-write demoted to offline-read fallback in the same pass. Completion screen CTA retargets here. Full phase table in [plans/my-trips-and-persistence-tieup.md](../plans/my-trips-and-persistence-tieup.md). **This is the next phase.**
