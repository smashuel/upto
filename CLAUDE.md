...
- **Basemap persisted on TripLink** — SHIPPED (2026-07-08, live-location Slice 04). Routes (`SerializableTrack`) and the chosen `plannedBasemap` (`MapLayer`) both ride the TripLink's JSONB `data`; view pages open on the planned canvas with a live-aware bounds-fit (no `plannedCamera` by design — a moving live marker would fight a frozen camera). See [brain/features/triplink-route-persistence.md](brain/features/triplink-route-persistence.md).
