...
- **Terrain-accurate picking** — `pickEllipsoid` → `scene.pickPosition` / `sampleTerrainMostDetailed`. Core picking + draw-time sampling shipped (commit `0834976`); finish/edit-commit settle heights before the route is emitted/persisted (2026-07-02, see [journal](../journal/2026-07-02-finish-settlement-race.md)); **settle window hardened + wizard upserts routes by id** (2026-07-03 — wizard submit waits out pending settles, every teardown strands every in-flight await, honest `settling` phase; see [journal](../journal/2026-07-03-settle-window-hardening.md) / [ADR 014](../decisions/014-settle-window-is-a-real-state.md)). Remaining slices: waypoint heights, honest degradation — [.scratch/terrain-accurate-picking/](../../.scratch/terrain-accurate-picking/PRD.md). (Stream 1.)
